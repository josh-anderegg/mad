...
for biome in os.listdir('data/clusters'):
    ...
    model_files = files[test_count:]
    
    result = subprocess.run(f'./lasso_train.py {' '.join(model_files)} -o "outputs/{biome_str}" -c {file_amount} -r {42} -p 10000' , shell=True, capture_output=True, text=True)
