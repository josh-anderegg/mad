...
gdf = gpd.read_file(BASE_DIR / 'data/maus/global_mining_polygons_v2.gpkg').to_crs(epsg=3857)
biomes = gpd.read_file(BASE_DIR / 'data/Ecoregions2017/Ecoregions2017.shp').to_crs(epsg=3857)
sindex = gdf.sindex
...
def process_cell(coord):
    ...
    if not idxs:
        ...
        if not overlap.empty and random.randrange(100) < 5: # Must not be ocean and pass 5% chance 
            return cell
        return None
...
grid_gdf = gpd.GeoDataFrame({'geometry': grid_cells}, crs='EPSG:3857').to_crs('EPSG:4326')
grid_gdf.to_file(BASE_DIR / f"data/grids/grid_{GRID_SIZE}_epsg3857.gpkg", layer='squares_layer', driver="GPKG")
...
grid_gdf[['min_lon', 'min_lat', 'max_lon', 'max_lat']].to_csv(BASE_DIR / f'data/grids/grid_{SEED}_{GRID_SIZE}_epsg3857.csv')